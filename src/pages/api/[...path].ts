...
async function handleTreehole(
	...
	if (method === "GET") {
		if (parts.length === 2) {
			...
			return json({ messages: visible, total: ids.length });
		}
	...
	if (method === "POST") {
		...
		if (parts.length === 4 && parts[3] === "resonance") {
			...
			return json({ id, resonance: msg.resonance });
		}
		...
		if (parts.length === 4 && parts[3] === "reply") {
			...
			return json({ id, content, hint: "回复成功" });
		}
	}
...
async function handleGuestbook(
	...
	if (method === "GET") {
		if (parts.length === 2) {
			...
			return json({ messages, total: ids.length });
		}
	...
	if (method === "POST") {
		...
		if (parts.length === 4 && parts[3] === "vote") {
			...
			if (!["agree", "disagree", "neutral"].includes(type)) {
				return json({ error: "Invalid vote type" }, 400);
			}
...
async function handleNotebook(
	...
	if (method === "POST" && parts.length === 2) {
		...
		if (!notebook || !title || !content) {
			return json({ error: "notebook, title, content are required" }, 400);
		}
...
async function handleActivity(
	...
	if (method === "GET") {
		if (parts.length === 2) {
			...
			if (slug) {
				...
				return json(normalizeActivity(activity));
			}
	...
	if (method === "POST") {
		if (parts.length === 2) {
			...
			return json(normalizeActivity(activity), 201);
		}
	}
