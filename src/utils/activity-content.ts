...
export function renderRichContent(markdown: string): string {
	...
	const flushQuote = () => {
		...
		blocks.push(`<blockquote>${quoteItems.map((item) => `<p>${renderInline(item)}</p>`).join("")}</blockquote>`);
		quoteItems = [];
	};
	...
	for (const rawLine of lines) {
		...
		if (image && SAFE_LINK_PATTERN.test(image[2])) {
			flushAll();
			const alt = escapeHtml(image[1] || "动态图片");
			const url = escapeHtml(image[2]);
			blocks.push(
				`<figure class="activity-rich-image"><img src="${url}" alt="${alt}" loading="lazy" decoding="async" /><figcaption>${alt}</figcaption></figure>`,
			);
			continue;
		}
